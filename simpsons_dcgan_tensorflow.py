...
def generator(z, training):
    with tf.variable_scope("generator", reuse=not training):
        ...
        trans_conv1 = tf.layers.conv2d_transpose(inputs=fully_connected,
                                                 ...
                                                 strides=[2, 2],
                                                 padding="SAME",
                                                 kernel_initializer=tf.random_normal_initializer(stddev=WEIGHT_INIT_STDDEV),
                                                 name="trans_conv1")
...
def model_loss(input_real, input_z):
    g_model = generator(input_z, True)
...
def test(sess, input_z, epoch):
    example_z = np.random.uniform(-1, 1, size=[SAMPLES_TO_SHOW, input_z.get_shape().as_list()[-1]])
    samples = sess.run(generator(input_z, False), feed_dict={input_z: example_z})
...
INPUT_DATA_DIR = "C:/Users/edwardhyde/PycharmProjects/simpsons/cropped/"
